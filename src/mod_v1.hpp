...
namespace v1 {
  ...
  private:
    ...
    using motion_map = std::unordered_map<
        contour_id,
        cdt::offset_t,
        std::hash<contour_id>,
        std::equal_to<contour_id>,
  ...
  public:
    ...
    [[nodiscard]] motion_map detect(source_t const& previous,
      ...
      auto marked{
          mark_motion(p_start, c_start, c_end, width, c_size, contours)};
  ...
  private:
    [[nodiscard]] markings mark_motion(cell_t const* previous,
      ...
      for (; current < end; current += width, previous += width) {
        for (auto c{current}, e{current + window}, p{previous}; c < e;
             ++c, ++p) {
    ...
    void process_window(cell_t const* start,
                        ...
                        std::uint8_t height,
                        std::size_t stride) {
      auto& counter{tracker_.try_emplace(ref.id_, tracker_.get_allocator())
                        .first->second};
